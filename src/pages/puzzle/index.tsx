...
const WordSearch: React.FC = () => {
    ...
    const placeWordsInGrid = (grid: Cell[][], words: string[]) => {
        const directions = [
            [0, 1],   // right
            [1, 0],   // down
    ...
    const highlightFoundWord = (word: string, grid: Cell[][]) => {
    ...
    const checkWordAtPosition = (word: string, startRow: number, startCol: number, dy: number, dx: number, grid: Cell[][]) => {
        if (
            startRow + dy * (word.length - 1) < 0 ||
            startRow + dy * (word.length - 1) >= grid.length ||
            startCol + dx * (word.length - 1) < 0 ||
